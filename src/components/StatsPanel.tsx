...
export default function StatsPanel({
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <h2 className="text-xl font-bold text-gray-900"><BarChartOutlined className="mr-2" />学习统计</h2>
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-medium text-gray-500">学习量</h3>
          ...
          {chartItems.map((item, i) => {
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center h-full justify-end">
                {/* 数值标注 */}
                <span className={`text-xs mb-1 ${item.isToday ? 'text-indigo-600 font-medium' : 'text-gray-500'}`}>
                  {total}
                </span>
                {/* 柱子 */}
                <div
                ...
                {/* 日期标签 */}
                <span className={`text-xs mt-1 ${item.isToday ? 'text-indigo-600 font-medium' : 'text-gray-400'}`}>
                  {item.label}
                </span>
              </div>
            );
